...
with open(file_to_load) as emp_data:
    ...
    for row in reader:
        ...
        ssn = str.replace(row[3], row[3][0:6], '***-**')
...
alldata = zip(emp_ids, emp_last_names, emp_first_names, emp_dobs, emp_ssns, emp_states)


# Write all of the election data to csv
with open(file_to_output, "w", newline ='') as output:
    writer = csv.writer(output)
    fieldnames=['ID', 'Last Name', 'First Name', 'DOB', 'SSN', 'State']
